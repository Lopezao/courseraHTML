...
var insertProperty = function (string, propName, propValue) {
    ...
    string = string.replace(new RegExp(propToReplace, "g"), propValue);//g is a flag, basicament diz para dar replace sempre que encontrar, como o sed s/old-text/new-text/g
    return string;
}

// Remove the class 'active' from home and switch Menu button
    function switchMenuToActive() {
        ...
        if (classes.indexOf("active") == -1) { // indexOf retorna -1 se nao encontrar "active"
            classes += " active";
...
$ajaxUtils.sendGetRequest(
    homeHtml, function (responseText){//this function is the handler
...
dc.loadMenuCategories = function(){
    showLoading("#main-content");
    $ajaxUtils.sendGetRequest(allCategoriesUrl, buildAndShowCategoriesHTML);// tue is default
};
...
for (var i = 0; i < menuItems.length; i++){
    ...
    // Add a clearfix after every second menu item
...
function insertItemPortionName(html, portionPropName, portionValue){
    // If not specifid, return original string
    if(!portionValue) {
